package aerys.minko.example.core.edgedetection.shaders {
	...
	public class EdgeDetectionShaderPart extends SamplingShaderPart {
		...
		private var e_kernel  	: SFloat = float2	( 1.5, 3 );
		...
		public function combineOutlines( normalOutline	: SFloat, depthOutline	: SFloat ) : SFloat{
			var w : SFloat = multiply(subtract(1, float2(normalOutline, depthOutline)), e_kernel);
			return add(w.x, w.y);
		}
		...
		public function normalDiscontinuityFilter( uv : SFloat ) : SFloat{
			...
			nd = subtract( nd , e_barrier.x);
			nd = greaterThan(nd, 0);
			return saturate(dotProduct4(nd, e_weights.x));
		}
		...
		public function depthFilter( uv : SFloat ) : SFloat{
			...
			dd = subtract(absolute(subtract(multiply(2, unpack(getPixel(_depthMap, uv, 0)).x), dd)), e_barrier.y);
			dd = greaterThan(0, dd);
			return saturate(dotProduct4(dd, e_weights.y));
		}
